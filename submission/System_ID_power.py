...
# %%
# Functions for power spectral density analysis

def analyze_power_spectral_density(data, plot=False):
    """Analyze power spectral density of input-output data.
    ...
    plot : bool, optional
        Whether to plot results. Defaults to False.
    
    Returns
    -------
    dict
        Dictionary containing PSD results and transfer function estimates
    """
    # Extract time
    t = data[:, 0]
    N = t.size
    ...
    u = data[:, 1]  # V, volts
    y = data[:, 2]  # LPM, liters per minute
    ...
    G_csd_masked = G_csd[mask]
    fcsd = f[mask]
    omega = omega_org[mask]

    # Plotting Units
    G_csd_mag = np.sqrt((np.real(G_csd_masked))**2 + (np.imag(G_csd_masked))**2)  # absolute
    G_csd_phase = np.arctan2(np.imag(G_csd_masked), np.real(G_csd_masked))  # rad
    ...
    return {
        ...
        'Cuy': Cuy,
        'G_csd': G_csd_masked,
        'G_csd_mag_dB': G_csd_mag_dB,
        'G_csd_phase_deg': G_csd_phase_deg,
        'mask': mask,
        'fs': fs,
        'fN': fN
    }
...
def fit_transfer_function_from_psd(psd_results, m=0, n=1, plot=False):
    """Fit a transfer function from PSD results.
    
    Parameters
    ----------
    psd_results : dict
        Results from analyze_power_spectral_density
    m : int, optional
        Numerator degree. Defaults to 0
    n : int, optional
        Denominator degree. Defaults to 1
    plot : bool, optional
        Whether to plot results. Defaults to False
    ...
    """
    omega = psd_results['omega']
    G_csd = psd_results['G_csd']
    Cuy = psd_results['Cuy']
    mask = psd_results['mask']
    fcsd = psd_results['fcsd']
    ...
    if plot:
        ...
        fig, axes = plt.subplots(2, 1, figsize=(height * gr, height))
        # Magnitude plot
        axes[0].semilogx(fcsd, psd_results['G_csd_mag_dB'], '.', color='C3', label='System ID')
        ...
        axes[0].legend(loc='best')
        # Phase plot
        axes[1].semilogx(fcsd, psd_results['G_csd_phase_deg'], '.', color='C3')
        ...
        ax[0].semilogx(omega, psd_results['G_csd_mag_dB'], '.', color='C3', label='System ID')
        ax[0].set_xlabel(r'$\omega$ (rad/s)')
        ax[0].set_ylabel(r'Magnitutde (dB)')
        ax[1].semilogx(omega, psd_results['G_csd_phase_deg'], '.', color='C3', label='System ID')
...
if __name__ == "__main__":
    ...
    psd_results = analyze_power_spectral_density(data_0, plot=True)
    
    # Fit transfer function
    m, n = 0, 1
    G_LS = fit_transfer_function_from_psd(psd_results, m=m, n=n, plot=True)
